...
interface AssetItem { url: string; taskType: string; createdAt: string }
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      const { data: tasks } = await supabase
        .from('tasks').select('task_type, output_urls, created_at')
        ...
        .limit(60)
      if (tasks) {
        const all = tasks.flatMap((t: { task_type: string; output_urls: string[]; created_at: string }) =>
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        ...
        <p className="text-brand-muted text-sm">共 {images.length} 张已生成图片</p>
      </div>
      {images.length === 0 ? (
          ...
          {images.map((img, i) => (
            <div key={i} className="group relative bg-white rounded-2xl border border-brand-border overflow-hidden">
              ...
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                <a href={img.url} download={`${TASK_LABELS[img.taskType] || img.taskType}_${i}.jpg`}
                  className="flex items-center gap-1.5 bg-white text-brand-text text-xs px-3 py-2 rounded-full font-medium">
                  <Download className="w-3.5 h-3.5" />下载
                </a>
              </div>
              <div className="p-3">
                <p className="text-xs font-medium text-brand-text">{TASK_LABELS[img.taskType] || img.taskType}</p>
