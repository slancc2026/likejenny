...
import { packagingPrompt } from '@/lib/prompts'
...
export async function POST(req: NextRequest) {
  ...
  const { data: task } = await supabase.from('tasks').insert({
    user_id: user.id, task_type: 'packaging', status: 'processing',
    input_data: { brand_name: brand.brand_name }, credits_cost: 20,
  }).select().single()
  if (!task) return NextResponse.json({ error: '创建任务失败' }, { status: 500 })

  try {
    await deductCredits(user.id, 'packaging', task.id)
    const prompt = packagingPrompt(brand)
    const [b64] = await generateImage({ prompt, size: '1024x1365', quality: 'high' })
    const url = await uploadGeneratedImage(user.id, 'packaging', task.id, b64, 0)
